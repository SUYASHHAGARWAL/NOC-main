            ...
            if record:
                ...
                if(record[0]['dept'] == 'Tnp'):
                    ...
                    if(record[0]['status'] == 'passive'):
                        ...
                        rec3['count(*)'] = 0
                        rec4['count(*)'] = 0
                        return render (req, "AdminDash.html",{'count':rec['count(*)'],'count2':rec2['count(*)'],'count3':rec3['count(*)'],'count4':rec4['count(*)'],'clicked':data,'dispname':dispname})
                    return render (req, "AdminDash.html",{'record':record[0],'count':rec['count(*)'],'count2':rec2['count(*)'],'count3':rec3['count(*)'],'count4':rec4['count(*)'],'clicked':data,'rrolee':rrolee,'naemme':naemme,'dispname':dispname})
                    ...
                    if(record[0]['status'] == 'passive'):
                        print("huiiiii")
                        return render (req, "AdminDash.html",{'count':0,'count2':0,'count3':0,'count4':0,'count5':0,'clicked':data,'dispname':dispname})
                    return render (req,"AdminDash.html",{'record':record[0],'count':len(r),'count2':len(r2),'count3':rec3['count(*)'],'count4':rec4['count(*)'],'count5':len(r3),'clicked':data,'rrolee':rrolee,'naemme':naemme,'dispname':dispname})
            else:
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if check_password(password, admin.Password):
                print("hello admin",admin.status)
...
def EditSaveDept(request):
    try:
        if request.method == 'POST':
            ...
            qry = "select * from nocrest_department where Dep_Id = '{0}'".format(stud_branch)
            cursor = connection.cursor()
            cursor.execute(qry)
            rec = tuple_to_dict.ParseDictMultipleRecord(cursor)
            ...
            if department == 'Tnp':
                    try:
                        if(approval == "Approved"):
                            ...
                            logo = 'nocrest/Static/Images/mitsheader.png'
                            fname = 'signature.png'
                            current_directory = os.getcwd()
...
def BonaEdit(request):
    try:
        if request.method == 'POST':
            ...
            approval = request.POST.get('approval')
            comment = request.POST.get('comment')
            print(approval, comment, stud_enr, stud_branch)
            qry = "select * from nocrest_department where Dep_Id = '{0}'".format(stud_branch)
            cursor = connection.cursor()
            cursor.execute(qry)
            rec = tuple_to_dict.ParseDictMultipleRecord(cursor)
            ...
            try:
                if request.POST['approval'] == 'Approved':
                        ...
                        Mitslogo = 'nocrest/Static/Images/mitsheader.png'
                        logo_path = os.path.join(current_directory,logo)
                        relative_path = "pdfs/"
                        ...
                        link = f"https://noc.mitsgwalior.in/verifyBonaf/?enrollment_number={apid}"  # Replace with your custom link
                        qr = qrcode.QRCode(
                                    version=1,
                                  error_correction=qrcode.constants.ERROR_CORRECT_L,
                                     box_size=10,
                        ...
                        qr.make(fit=True)
                        qr_img = qr.make_image(fill_color="black", back_color="white")
                        qr_img_path = f"nocrest/Static/Images/bonaqrcode_{deptid}.png"  # Replace with the path to save the QR code image
                        qrrpath = os.path.join(current_directory,qr_img_path)
                        qr_img_path = qrrpath
                        qr_img.save(qr_img_path)
                        print(qr_img_path)
                        filename = f'{stud_name}_{stud_enr}_Bonafode.pdf'
...
def Status(request):
      try:
        if request.method == 'POST':
            ...
            cat.dept = request.POST.get('dept')
            cat.save()
            print("saved")
            return redirect('/api/superadmin')
      except Exception as e:
...
@api_view(['GET', 'POST', 'DELETE'])
def verifyBonaf(request):
    try:
        if request.method == 'GET':
            enrollment_number = request.GET.get('enrollment_number', None)
            if enrollment_number:
                qry = "select * from nocrest_bonafidemodel where app_id = '{0}' and dept_approval = 'Approved'".format(1)
                cursor = connection.cursor()
                cursor.execute(qry)
                rec = tuple_to_dict.ParseDictMultipleRecord(cursor)
                print("Record",rec)
                return render(request, 'BonafVerify.html', {'data': rec})
            else:
                return render(request, 'BonafVerify.html', {'error': 'Enrollment number not provided'})
    except Exception as e:
        # Handle other exceptions appropriately
        return render(request, 'verify_noc.html', {'error': f"Unexpected error: {e}"})
...
def StudentProfile(req):
    try:
            ...
            branchstud = r['Department_name']
            return render(req, 'Profile.html', {'record': rec, 'branchstud': branchstud})
    except Exception as e:
...
def EditDeptAdmin(req):
    try:
        ...
        if req.method == 'POST':
            ...
            dispname = req.POST.get('Displayname')
            ...
            Application_edit.Displayname = dispname
            Application_edit.save()
